...
$pRes         = mysqli_query($conn, "SELECT name, profile_photo, email FROM users WHERE id='$uid'");
...
// Derive college domain from session or DB
$domain = $_SESSION['domain'] ?? '';
if (empty($domain) && !empty($pRow['email'])) {
    $parts  = explode('@', $pRow['email']);
...
<title>Upload Item — Back2U</title>
<script>
(function () {
...
*,*::before,*::after{margin:0;padding:0;box-sizing:border-box;transition:background-color 0.3s,color 0.2s,border-color 0.2s;}
body{font-family:'Segoe UI',sans-serif;background:var(--bg);color:var(--text);min-height:100vh;}
header{background:var(--header-bg);color:var(--header-text);padding:15px 20px;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:10;box-shadow:0 2px 10px var(--shadow);}
...
/* College domain badge in header */
.domain-badge{background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.25);padding:4px 12px;border-radius:20px;font-size:0.78rem;color:rgba(255,255,255,0.85);}

.page{max-width:780px;margin:40px auto;padding:0 20px 60px;}
.back-link{display:inline-flex;align-items:center;gap:6px;color:var(--accent);text-decoration:none;font-size:0.9rem;margin-bottom:24px;}
.back-link:hover{text-decoration:underline;}
.mode-selector{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:32px;}
.mode-card{background:var(--surface);border:2px solid var(--border);border-radius:12px;padding:28px 20px;text-align:center;cursor:pointer;transition:border-color 0.2s,transform 0.2s,box-shadow 0.2s,background-color 0.3s;}
.mode-card:hover{border-color:var(--accent);transform:translateY(-2px);box-shadow:0 4px 15px var(--shadow);}
...
.form-card{background:var(--surface);border-radius:12px;box-shadow:0 2px 10px var(--shadow);padding:32px;display:none;border:1px solid var(--border);}
...
.form-group label{display:block;font-weight:600;color:var(--text);margin-bottom:6px;font-size:0.9rem;}
.form-group input[type=text],.form-group textarea{width:100%;padding:11px 14px;border:1px solid var(--input-border);border-radius:6px;font-size:0.95rem;font-family:inherit;background:var(--input-bg);color:var(--text);}
.form-group input:focus,.form-group textarea:focus{border-color:var(--accent);outline:none;box-shadow:0 0 0 3px rgba(74,128,212,0.15);}
.form-group textarea{resize:vertical;min-height:90px;}
.upload-zone{border:2px dashed var(--border);border-radius:8px;padding:30px;text-align:center;cursor:pointer;position:relative;background:var(--surface2);transition:border-color 0.2s,background-color 0.3s;}
...
.submit-btn{background:var(--accent);color:white;border:none;padding:13px 32px;border-radius:6px;font-size:1rem;font-weight:600;cursor:pointer;width:100%;margin-top:8px;transition:background 0.2s;}
...
<div class="page">
  ...
  <p class="page-subheading">
    Items will be visible only to students at
    <strong><?php echo htmlspecialchars($domain ?: 'your college'); ?></strong>.
  </p>
  ...
  <div class="form-card" id="form-register">
    ...
    <p class="subtitle">Register your item now. If it gets lost later, mark it as lost from My Items.</p>
    <div class="info-box">
  ...
  <div class="form-card" id="form-found">
    ...
    <div class="info-box">
      ℹ️ After submitting, you'll see a list of <strong>lost items</strong> from your college. Check if any match what you found, then chat with the owner.
    </div>
    <form method="POST" enctype="multipart/form-data">
      <input type="hidden" name="mode" value="found_report">
      <div class="form-group">
        <label>Description of what you found *</label>
