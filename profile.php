...
<title>My Profile — Back2U Lost &amp; Found</title>
<script>
...
.profile-hero {
    ...
    box-shadow: 0 2px 10px var(--shadow); padding: 30px;
...
.profile-info h2 { font-size: 1.6rem; color: var(--accent); margin-bottom: 4px; }
.profile-info p  { color: var(--text-muted); margin-bottom: 4px; }
...
[data-theme="dark"] .stat-card.lost    .stat-num { color: #ef9a9a; }
[data-theme="dark"] .stat-card.found   .stat-num { color: #a5d6a7; }
[data-theme="dark"] .stat-card.claimed .stat-num { color: #90caf9; }
[data-theme="dark"] .stat-card.pending .stat-num { color: #ffe082; }

/* ── Panels ── */
.panel {
    background: var(--surface); border-radius: 12px;
    box-shadow: 0 2px 8px var(--shadow); margin-bottom: 25px;
    overflow: hidden; border: 1px solid var(--border);
}
.panel-header {
    background: var(--header-bg); color: white;
    padding: 15px 20px; font-size: 1rem; font-weight: bold;
}
...
.btn {
    background: var(--accent); color: white; border: none;
    padding: 11px 25px; border-radius: 6px; cursor: pointer; font-size: 0.95rem;
...
.item-table td {
    ...
    font-size: 0.9rem; vertical-align: middle; color: var(--text);
}
...
[data-theme="dark"] .status-found      { background: #0d2e18; color: #a5d6a7; }
...
<header>
  ...
  <div class="header-right">
    ...
    <a href="dashboard.php" style="color:var(--header-text);text-decoration:none;margin-right:5px;">Dashboard</a>
...
<div class="container">
  ...
  <div class="profile-hero">
    ...
    <div class="profile-info">
      ...
      <p style="margin-top:8px;color:var(--text-faint);font-size:0.85rem;">Member · Back2U Lost &amp; Found</p>
    </div>
  </div>
  ...
  <div class="stats-row">
    <div class="stat-card lost">
      <div class="stat-num"><?php echo intval($stats['lost_count']); ?></div>
      <div class="stat-label">Items Lost</div>
    </div>
    <div class="stat-card found">
      <div class="stat-num"><?php echo intval($stats['found_count']); ?></div>
      <div class="stat-label">Items Found</div>
    </div>
    <div class="stat-card claimed">
      <div class="stat-num"><?php echo intval($stats['claimed_count']); ?></div>
      <div class="stat-label">Claimed</div>
    </div>
    <div class="stat-card pending">
      <div class="stat-num"><?php echo intval($stats['pending_count']); ?></div>
  ...
  <div class="panel">
    ...
    <div class="panel-body" style="padding:0;">
      <?php if (empty($myItems)): ?>
        <p style="padding:25px;color:var(--text-faint);text-align:center;">You haven't uploaded any items yet.</p>
      <?php else: ?>
      <table class="item-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td style="color:var(--text-muted);"><?php echo htmlspecialchars(substr($item['description'] ?? '', 0, 60)); ?>...</td>
...
/* Preview new photo locally before form submits */
